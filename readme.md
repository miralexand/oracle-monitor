...
| 日志细分 | 按级别、数据库、关键字筛选查看，点击级别卡片快速查看对应日志，可导出筛选结果 |
...
1. **监控面板**：整体健康状态、服务运行时间、各库可用率环形图、响应时间曲线、24h/7d 告警趋势，可忽略指定告警；24h 连接失败次数单独统计与展示；点击统计卡按状态筛选、点击数据库卡跳转编辑、响应时间图点击放大，均带非线性（弹性/缓动）动画
2. **数据库管理**：添加任意多个 Oracle 数据库（名称 / 主机 / 端口 / Service Name 或 SID / 账号）
3. **设置**：配置 SMTP、检测间隔、阈值、监控模块开关、Thick 模式
4. **日志**：按状态级别 / 数据库 / 关键字筛选，点击级别卡片快速查看对应级别日志，导出筛选结果
...
| v1.2.1 | 2026-09-13 | 修复忽略告警后状态未回归正常；24h 异常改为连接失败次数并单独展示；面板卡片可点击跳转/筛选与响应时间图放大（非线性动画）；日志页点击级别卡片筛选 |
